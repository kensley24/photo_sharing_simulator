...
void AlbumCreator::addAlbum(){
    string title, descr;
    view.promptAlbum(title, descr);
    pg.addAlbum(title, descr);
}
...
void AlbumCreator::getPhotosByCandD(){
    ...
    Criteria* crit = mf.dateAndCatCriteria(after, before, cat);
    cout<<*crit<<endl;
    currentPhotos.clear();
...
void AlbumCreator::getPhotosByCat(){
    string cat;
    view.promptCategory(cat);
    Criteria* crit = mf.categoryCriteria(cat);
    cout<<*crit<<endl;
    currentPhotos.clear();
    pg.getPhotos(*crit, currentPhotos);
    delete crit;
...
void AlbumCreator::getPhotosByDate(){
    ...
    Criteria* crit = mf.dateCriteria(before, after);
    cout<<*crit<<endl;
    currentPhotos.clear();
    pg.getPhotos(*crit, currentPhotos);
    delete crit;
...
void AlbumCreator::printCurrentPhotoList(){
    cout<<"Photo list size: "<<currentPhotos.getSize()<<endl;
    view.printPhotos(currentPhotos);
}

void AlbumCreator::displayCurrentPhotoList(){
    for (int i = 0; i < currentPhotos.getSize(); ++i){
